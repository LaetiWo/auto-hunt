  ...
  return (
    <div className="w-full pt-4 pb-14">
      <div className="w-full max-w-7xl mx-auto space-y-6">
          ...
          {BRANDS.map((item, index) => (
            <button
              ...
              className={cn(
                ...
                px-3 border-b-2 border-transparent hover:border-primary hover:text-primary`,
                item.value === active && "text-primary",
        ...
        {isPending ? (
          <CarListingSkeleton
            className="grid grid-cols-1 min-[500px]:grid-cols-2 lg:grid-cols-3 
            xl:grid-cols-4 gap-4 sm:gap-6 px-4 sm:px-6 md:px-0"
            layout="grid"
          />
        ) : listings?.length === 0 ? (
          <div>
            <EmptyState message="Aucun véhicule disponible." />
          </div>
        ) : (
          <div
            className="grid grid-cols-1 min-[500px]:grid-cols-2 lg:grid-cols-3 
        ...
        <div className="flex mt-4 justify-center ">
          <Link href="/search">
            <Button
              size="lg"
              className="uppercase font-semibold py-3 px-10 rounded-lg !h-auto bg-primary hover:bg-primary/80 "
            >
              Voir plus
            </Button>
          </Link>
